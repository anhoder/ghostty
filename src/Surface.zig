        ...
        if (self.keyboard.sequence_set) |set| {
            ...
            if (set.getEvent(event)) |v| {
                break :leaf switch (v.value_ptr.*) {
                    .leader => |seq_set| {
                        // Store this event so that we can drain and encode on
                        // invalid. We don't need to cap this because it is
                        // naturally capped by the config validation.
                        ...
                        // Setup the next set we'll look at only after all
                        // fallible work.
        ...
        if (table_items.len > 0) {
            for (0..table_items.len) |i| {
                ...
                if (table.set.getEvent(event)) |v| {
                    ...
                    break :leaf switch (v.value_ptr.*) {
                        .leader => |seq_set| {
                            // Store this event so that we can drain and encode
                            // on invalid. We don't need to cap this because it
                            // is naturally capped by the config validation.
                            if (try self.encodeKey(event, insp_ev)) |req| {
                                self.keyboard.sequence_queued.append(self.alloc, req) catch |err| {
                                    req.deinit();
                                    return err;
