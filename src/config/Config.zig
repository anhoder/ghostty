...
pub const Keybinds = struct {
    ...
    pub fn formatEntryDocs(self: Keybinds, formatter: formatterpkg.EntryFormatter, docs: bool) !void {
        ...
        // Format conditional bindings from the separate conditional_bindings list.
        // These are emitted with their [condition] prefix (e.g. [process=vim]).
        for (self.set.conditional_bindings.items) |entry| {
            var writer: std.Io.Writer = .fixed(&buf);
            // Write [condition]trigger=action into buf
